## Intro
---
-
## Metrics
---
-
## Since Last We Met
---
### Shawn K.
- [[2582-elastic-server-for-analytics]]
- Wrapped up new plan ranking algo
- 
### Andre R.
- [[2205-add-notes-section-in-the-back-end-for-build-photo-albums]]
- [[2503-related-plans-cta-popover-to-pdp]]
- [[200-order-revisions-before-releasing-to-designer]]
	- Moved to review
### Craig B.
- Doing mix of things
- Looking at assigned tasks to see if can do anything, build test plan
- A little automated tests work
### Steven R.
- [[2493-upgrade-shipping]] Moved to review
- [[2526-customer-receipt-delivery-delay-notices]] Moved to review
- [[2574-sanitize-query-parameters-for-save-this-search]]
	- Trying to wrap up today
- [[2396-search-facet-analytics-report-fixes-updates]]
### Brian M.
- [[2075-css-framework]]
### Taylor S.
- [[2580-zero-downtime-high-availability-plan]]
- [[2596-db-performance-helper]]
- [[2592-show-republished-plans]]
- [[2578-add-front-tag-for-georgia-orders]]
- [[2599-zoom-fathom-for-brian]]
## Rock Review
---
-
## Headlines
---
-
## Issues
---
-
## Ratings
---
- Shawn K.:
- Andre R.:
- Craig B.:
- Brian M.:
- Taylor S.:
- Steven R.:

Average:
## Other Notes
---
Weather
- Shawn K.: upper 70s, going back into 60s
- Andre R.: 70, cloudy; sure will drop back down to 50s
- Craig B.: 66, feels like 75
- Brian M.: 40s, cold
- Taylor S.:
- Steven R.: 72, sunny

- Brian starts Fathom and does not ask for consent, running from Brian's Zoom, no Taylor in meeting
	- Taylor in other meeting about reports
	- Planning for tradeshow
	- Lighter week, not a lot of tasks picked for dev so far
- Brian says that upcoming change might be to include developer in review process for larger tasks
- Brian says has not done much server config since php5
- Brian says doesn't like when dev says "you aren't going to like this, but I did what you asked"